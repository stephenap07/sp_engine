...
bool MD5Animation::LoadAnimation(const std::string &filename)
{
	...
	joint_infos.clear(); bounds.clear();
	base_frames.clear(); frames.clear();
	...
	while (!file.eof()) {
		if (param == "MD5Version") {
			...
			for (int i = 0; i < num_joints; i++) {
				JointInfo joint;
				file >> joint.name >> joint.parent_id >> joint.flags >> joint.start_index;
			...
			for (int i = 0; i < num_joints; i++) {
				...
				file >> junk;
				file >> base_frame.pos.x >> base_frame.pos.y >> base_frame.pos.z;
				file >> junk >> junk;
				file >> base_frame.orient.x >> base_frame.orient.y >> base_frame.orient.z;
				file.ignore(file_length, '\n');
                                      ...
                                      const FrameData &frame_data)
{
	...
	for (unsigned i = 0; i < joint_infos.size(); i++) {
		...
		if (joint_info.flags & 1) {
			animated_joint.pos.x = frame_data.data[joint_info.start_index + j++];
		}
		if (joint_info.flags & 2) {
			animated_joint.pos.y = frame_data.data[joint_info.start_index + j++];
		}
		if (joint_info.flags & 4) {
			animated_joint.pos.z = frame_data.data[joint_info.start_index + j++];
		}
		if (joint_info.flags & 8) {
			animated_joint.orient.x = frame_data.data[joint_info.start_index + j++];
		}
		if (joint_info.flags & 16) {
			animated_joint.orient.y = frame_data.data[joint_info.start_index + j++];
		}
		if (joint_info.flags & 32) {
			animated_joint.orient.z = frame_data.data[joint_info.start_index + j++];
		}

		ComputeQuatW(animated_joint.orient);

		if (animated_joint.parent >= 0) {
			SkeletonJoint &parent_joint = skeleton.joints[animated_joint.parent];
...
void MD5Animation::Update(float dt)
{
	...
	while (anim_time > anim_duration) anim_time -= anim_duration;
	while (anim_time < 0.0f) anim_time += anim_duration;
	...
	InterpolateSkeletons(animated_skeleton, skeletons[frame0], skeletons[frame1], interpolate);
}
