...
//------------------------------------------------------------------------------
void Camera::MoveForward(float dt) { pos += 10.0f * dt * dir; }

//------------------------------------------------------------------------------
void Camera::MoveBackward(float dt) { pos -= 10.0f * dt * dir; }

//------------------------------------------------------------------------------
void Camera::MoveRight(float dt) { pos += 10.0f * dt * glm::cross(dir, up); }

//------------------------------------------------------------------------------
void Camera::MoveLeft(float dt) { pos -= 10.0f * dt * glm::cross(dir, up); }
...
void Camera::HandleMouse(int x, int y, float dt)
{
    ...
    glm::quat heading_quat =
        glm::angleAxis(-10.0f * x * dt, glm::vec3(0.0f, 1.0f, 0.0f));
