...
void Console::Init(float window_width, float window_height)
{
    ...
    frame.Init(0, 0, 2.0f / window_width, 2.0f / window_height, window_width,
               0.0f);
    text_box.Init(0, 0, 2.0f / window_width, 2.0f / window_height, window_width,
                  15.0f);
...
void Console::OpenFrame() { is_active = true; }
...
void Console::Draw()
{
    if (is_active) {
        ...
        for (int i = 0; i < cmd_count; i++) {
            text_def->DrawText(command_history[i], 0,
                               frame.GetHeight() - 35.0f * (cmd_count - i));
        }
    }
}
...
void Console::SetText(const std::string &text) { console_text = text; }

//=============================================================================

const std::string Console::GetText() const { return console_text; }
