...
{
    GLuint ret = MakeTexture(name, GL_TEXTURE_2D);
    if (!ret) {
...
bool loadiqmmeshes(const char *filename, const iqmheader &hdr, uchar *buf)
{
    ...
    for(int i = 0; i < (int)hdr.num_meshes; i++)
    {
        ...
        textures[i] = loadtexture(&str[m.material]);
...
void initgl()
{
    ...
    notexture = loadtexture("notexture.tga");
}
        ...
        while(SDL_PollEvent(&window_ev)) {
            switch(window_ev.window.event) {
                case SDL_WINDOWEVENT_RESIZED:
                    reshapefunc(window_ev.window.data1, window_ev.window.data2);
                    break;
            }
